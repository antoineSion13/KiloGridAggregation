...
uint32_t pleave_sampling_duration = 32*__sampling_time__; //2 seconds sampling
int robot_type = INFORMED_WHITE; //DEFINE HERE THE TYPE OF ROBOT
double alpha = __alpha_param__;
double beta = __beta_param__;
...
void loop() {
  ...
  // debug_info_set(timer_go_straight_debug, timer_go_straight);
    ...
    set_color(RGB(3, 3, 3));
      ...
      if(kilo_ticks >= timer_leave) //if it is time to sample the probability to leave
      {
        ...
        if(leave_site())
        {
          ...
          timer_leave = 0;
          timer_stuck_in_site = kilo_ticks + 32*30; //30 seconds to get out the site : if stuck by other kilobots, cannot get out, go to state stay again
          move_straight();
          timer_turn = 0;
        }
        else
    ...
    set_color(RGB(0, 3, 0));
      ...
      else if(is_in_site() && kilo_ticks >= timer_stuck_in_site)
      {
        stop();
        state = STAY;
        entering_site = false;
